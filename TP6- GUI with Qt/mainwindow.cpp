...
{
    connect(gestionnaire_, &Gestionnaire::usagerAjoute, this, &MainWindow::usagerAEteAjoute);
    connect(gestionnaire_, &Gestionnaire::usagerSupprime, this, &MainWindow::usagerAEteSupprime);
}
...
void MainWindow::setUI() {
    ...
    QStringList list;
    list << "Tout Afficher" << "Afficher Clients Reguliers" << "Afficher Clients Premiums"
         <<"Afficher Fournisseurs";
    showCombobox->addItems(list);
    ...
    // Vertical : ajoute un par-dessus l'autre
    ...
    // Horizontal : on les ajoute un a cote de l'autre
...
void MainWindow::selectionnerUsager(QListWidgetItem* item) {
    ...
    if(typeid(*usager) == typeid(ClientPremium)){
        editeurJoursRestants->setText(QVariant(static_cast<ClientPremium*>(usager)->obtenirJoursRestants()).toString());}
...
{
  int nb= gestionnaire_->obtenirUsagers().size();
  vector<Usager*> vectorUsagers = gestionnaire_->obtenirUsagers();
    for(int i=0;i<nb;i++)
    {
        gestionnaire_->supprimerUsager(vectorUsagers[i]);
    }
}

//Supprime l'usager sélectionné dans la liste
void MainWindow::supprimerUsagerSelectionne() {
    gestionnaire_->supprimerUsager(listUsager->currentItem()->data(Qt::UserRole).value<Usager*>());
...
void MainWindow::ajouterUsager() {
    ...
    list<QRadioButton*>::iterator it = boutonRadioTypeUsager.begin();
    list<QRadioButton*>::iterator end = boutonRadioTypeUsager.end();
    while(it != end)
    {
        if((*it)->isChecked())
    ...
    //Utilisation d'un try throw catch pour faire un popup message si tous les champs ne sont pas remplis
        ...
        try{
            ...
            {
                ExceptionArgumentInvalide nom(QString::fromStdString("Erreur: Le champ Nom est invalide."));
                throw nom;
            }
           else if (editeurPrenom->text().isEmpty())
            {
                ExceptionArgumentInvalide prenom(QString::fromStdString("Erreur: Le champ Prenom est invalide."));
                throw prenom;

            }
            else if (editeurIdentifiant->text().isEmpty())
            {
                ExceptionArgumentInvalide identifiant(QString::fromStdString("Erreur: Le champ Identifiant est invalide."));
                throw identifiant;

            }
            else if (editeurCodePostal->text().isEmpty())
            {
                 ExceptionArgumentInvalide postal(QString::fromStdString("Erreur: Le champ Code Postal est invalide."));
                 throw postal;

            }
            else if(  (*it)->text()=="&ClientPremium" && editeurJoursRestants->text().isEmpty())
            {
                 ExceptionArgumentInvalide jour(QString::fromStdString("Le champ Jours Restants est invalide"));
                 throw jour;
            }
       }

        catch (ExceptionArgumentInvalide& exc)
        {
            afficherMessage(exc.what());

            //tant qu'on a une exception, on ne veut pas que l'usager soit ajouté
            //on sort donc de cette méthode en attendant que l'utilisateur entre un
            //argument valide (on attend le prochain SIGNAL)
            return;
        }


    // On créé le bon type d'usager selon le cas
    if((*it)->text()=="&ClientPremium")
    {  nouvelUsager=new ClientPremium(editeurNom->text().toStdString(),editeurPrenom->text().toStdString(),
                                    editeurIdentifiant->text().toInt(),editeurCodePostal->text().toStdString(),
                                    editeurJoursRestants->text().toUInt());

    }

    else if((*it)->text()=="&Client")
    {
        nouvelUsager=new Client(editeurNom->text().toStdString(),editeurPrenom->text().toStdString(),
                                    editeurIdentifiant->text().toInt(),editeurCodePostal->text().toStdString());


    }
    else if((*it)->text()=="&Fournisseur")
    {nouvelUsager=new Fournisseur(editeurNom->text().toStdString(),editeurPrenom->text().toStdString(),
                                    editeurIdentifiant->text().toInt(),editeurCodePostal->text().toStdString());}
