...
*
* 1/
* La sortie du code est : 1
*
* 2/
* La méthode A::f() n'est pas déclarée virtuelle. Cela veut dire que si on utilise un pointeur d'une de ses classes dérivées
* et qu'on appelle f(), c'est la méthode de la classe de base qui sera appelée.
*
* 3/
* On peut soit ne pas déclarer la méthode B::f() virtuelle ou appeler directement v[0]->B::f().
*
* 4/
* Puisque la classe Usager est abstraite : au moins une de ces méhtodes est virtuelle pure. Ainsi, cela veut dire que cette méthode représente des objets trop généraux
* pour être instanciés : on a besoin de redéfinir spécifiquement certaines méthodes dans les classes dérivées.
*
* 5/
* Prenons l'exemple selon lequel on veut afficher les différents produits contenus dans le panier d'un client. On veut afficher séparément les Produits et les Produits Soldés.
* Pour se faire, on peut utiliser l'exemple de code suivant :
*
* cout << "PANIER (de " << obtenirNom() << ")"
*		<< "\n";
*
*	//On affiche d'abord les produits qui ne sont pas des Produits Soldés
*	for (unsigned int i = 0; i < panier_.size(); i++)
*	{
*		//Affiche les produits qui ne sont pas des Produits Soldés
*		if(dynamic_cast<Solde*>(panier_[i]))==nullptr)
*		{	panier_[i]->afficher();
*			cout << endl;
*		}
*	}
* // On affiche ensuite les produits qui sont des Produits Soldés
*	for (unsigned int i = 0; i < panier_.size(); i++)
*	{
*		//Affiche les produits qui ne sont pas des Produits Soldés
*		if (dynamic_cast<Solde*>(panier_[i])) != nullptr)
...
*	}
*
* On s'assure ainsi de bien séparer les différents types de produits en faisant recours à l'héritage multiple.
**/
