...
class Afficher {
public:
	void operator ()(const pair <Produit, int>& eq) {
		cout << eq.first.obtenirNom() << eq.second << endl;
	}
};
/*void Afficher(const pair <Produit, int>& eq) {
	cout << eq.first.obtenirNom() << eq.second << endl;
};
*/
class Trier {
public:
	bool operator< (const Produit& eq1, const	Produit& eq2) {
		return (eq1.obtenirNom() < eq2.obtenirNom());
	}
};
...
/*bool foncteurMin(const int& e1, const int& e2) {
	return (e1 < e2);
};*/
int main(){
map<Produit,int> ligue;
//map<Produit, int, Trier> co;
ligue.insert(make_pair(Produit(nullptr,(string)"allo"), 1));
ligue.insert(make_pair(Produit(nullptr, (string)"salut"), 2));
ligue.insert(make_pair(Produit(nullptr, (string)"bonjour"), 3));
//for (auto it = ligue.begin(); it != ligue.end(); it++)
//for_each(co.begin(), co.end(), Afficher());
//vector <pair <Produit, int>> copyligue;
//copy(ligue.begin(), ligue.end(), back_inserter(copyligue));
//sort(copyligue.begin(), copyligue.end(), Trier());
	/*list<int> L;
	L.push_back(1);
	L.push_back(-7);
	L.push_back(78);
	...
	L.push_back(96);
	L.push_back(32);
	list<int>::iterator it3 = find_if(
		L.begin(),
		L.end(),
		bind(logical_not<bool>(), bind(logical_and<bool>(), bind(greater_equal<int>(), std::placeholders::_1, 5), bind(greater_equal<int>(), 90, std::placeholders::_1))));
	cout << *it3 << endl;*/
	return 0;
}
